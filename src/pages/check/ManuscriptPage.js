...
const ManuscriptPage = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      align: "center",
      render: (text, record) => {
        return <Tag color={getColor(record.statusNum)}>{text}</Tag>;
      },
    },
    {
      title: "更新时间",
  ...
  const handleStepChange = async (delta, status, type) => {
    ...
    try {
      ...
      if (response.code === 200) {
        const newStatusName = stepsData[newStatusNum - 1]?.title || "";

        setManuscriptArr((prevArr) =>
          prevArr.map((item) =>
            item.id === status.id
              ? { ...item, statusNum: newStatusNum, statusName: newStatusName }
              : item
          )
        );
  ...
  const getColor = (statusNum) => {
    ...
    const match = colorMap.find(
      (item) => statusNum >= item.start && statusNum <= item.end
    );
    ...
    >
      ...
      >
        <div className="manuscript-detail">
          <Row gutter={[16, 16]}>
            <Col span={24}>
              <Text strong>稿件标题：</Text>
              {currentManuscript.manuscriptTitle}
            </Col>
            <Col span={12}>
              <Text strong>稿件标识：</Text>
            ...
            <Col span={24}>
              ...
              >
                ...
                >
                  {Array.isArray(currentManuscript.imgs) &&
                  currentManuscript.imgs.length > 0 ? (
          ...
          <Space size="middle" wrap>
            ...
            <Space size="small">
              <Button
                ...
                disabled={
                  currentStep === 0 || currentStep === stepsData.length - 1
                }
            ...
            <Select
              ...
              placeholder="选择跳转步骤"
              onChange={(value) => {
                handleStepChange(value, status, "rollBack");
              }}
            ...
            {/* 右侧关闭按钮 */}
            <Button onClick={() => setShow(false)} icon={<CloseOutlined />}>
              关闭
            </Button>
          </Space>
        }
